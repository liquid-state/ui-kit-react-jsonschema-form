...
export function getWidget(schema, widget, registeredWidgets = {}) {
  ...
  function mergeOptions(Widget) {
    ...
    if (!Widget.MergedWidget) {
      ...
      const MergedWidget = ({ options, ...props }) => (
        <Widget options={{ ...defaultOptions, ...options }} {...props} />
      );

      MergedWidget.propTypes = {
        options: PropTypes.object,
      };

      MergedWidget.defaultProps = {
        options: {},
      };

      Widget.MergedWidget = MergedWidget;
    }
...
export function mergeObjects(obj1, obj2, concatArrays = false) {
  ...
  return Object.keys(obj2).reduce((ac, key) => {
    const left = obj1[key];
    const right = obj2[key];
    if (key in obj1 && isObject(right)) {
      ac[key] = mergeObjects(left, right, concatArrays);
    } else if (concatArrays && Array.isArray(left) && Array.isArray(right)) {
      ac[key] = left.concat(right);
    } else {
      ac[key] = right;
    }
    return ac;
      ...
      if (schema.minItems) {
        if (!isMultiSelect(schema, definitions)) {
          ...
          if (schema.minItems > defaultsLength) {
            ...
            const fillerEntries = new Array(schema.minItems - defaultsLength)
              .fill(computeDefaults(schema.items, schema.items.defaults, definitions));
...
export function isSelect(_schema, definitions = {}) {
  ...
  if (Array.isArray(schema.enum)) {
    ...
    return altSchemas.every(altSchemas1 => isConstant(altSchemas1));
  }
  return false;
}
...
export function optionsList(schema) {
  ...
  const altSchemas = schema.oneOf || schema.anyOf;
  return altSchemas.map((schema1) => {
    const value = toConstant(schema1);
    const label = schema1.title || String(value);
    return { label, value };
  });
}
...
function withExactlyOneSubschema(
  ...
  const validSubschemas = oneOf.filter((subschema) => {
    ...
    if (!conditionPropertySchema) return false;
...
export function deepEquals(a, b, ca = [], cb = []) {
  ...
  if (a === b) {
    ...
    return deepEquals(Array.prototype.slice.call(a), Array.prototype.slice.call(b), ca, cb);
  }
